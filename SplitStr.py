...
def find_end(start_, arg, black, white, black_max, white_max, width):
    ...
    for m in range(start_ + 1, width - 1):
        if (black[m] if arg else white[m]) > (0.94 * black_max if arg else 0.94 * white_max):  # 0.94这个参数请多调整，对应下面的0.04
            end_ = m
            break
    return end_
...
class Setloader():
    ...
    def trainset_loader(self):
        path = 'D:\\code\\Python\\LicensePlateRecognitionSystem\\DataSet\\Trainsmall_root'
        trainset = datasets.ImageFolder(root=path, transform=transform)
        trainloader = torch.utils.data.DataLoader(trainset, batch_size=64, shuffle=True, num_workers=2)
        return trainloader

    def testset_loader(self):
        path = 'D:\\code\\Python\\LicensePlateRecognitionSystem\\test_char_root'
...
class Trainandsave():
    ...
    def load_test(self, img):
        ...
        input = input.unsqueeze(0)  # 增加一个维度，在原本第0维的位置增加一个维度使其符合pytorch的格式[B,C,H,W]
    ...
    for file in os.listdir(plate_dir):
        ...
        if len(OtherImg) == 6:
            ...
            for i in range(0, 6):
                ...
                cv2.imwrite(
                    'test_char_root/' + str(index) + '/' + imgname + words[i] + str(random.randint(1, 20)) + '.jpg',
                    OtherImg[i])
                if len(ChineseImg) != 0:
...
# 指定imgname，img
# 返回 imgname：文件名，binary_img：车牌经过处理后的二值图，SplitedImg：分割后的图片，resPlate：识别出的车牌结果
def SinglePlateSplit(imgname, img):
    # 字母字符图像列表
    OtherImg = []
    result = []
...
def SplitStr(path):
    ...
    # 指定imgname，img，返回 imgname：文件名，binary_img：车牌经过处理后的二值图，SplitedImg：分割后的图片，resPlate：识别出的车牌结果
    imgname, binary_img, SplitedImg, resPlate = SinglePlateSplit(imgname, img)
    # 指定路径，返回车牌名
...
# Split()
# # 以下代码用于建立测试集文件夹
# testRoot = 'D:\\CCPD2019\\ccpd_base\\' #这里需要在CCPD自己下载数据集（文件大小100多G）
